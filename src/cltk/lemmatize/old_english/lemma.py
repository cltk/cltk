...
class OldEnglishDictionaryLemmatizer:
    ...
    def lemmatize(self, text, best_guess=True, return_frequencies=False):
        ...
        if isinstance(text, str):
            ...
            raise TypeError(
                "lemmatize only works with strings or lists of string tokens."
            )

        return [
            self._lemmatize_token(token, best_guess, return_frequencies)
            for token in tokens
        ]
    ...
    def evaluate(self, filename):
        ...
        with open(filename, "r") as infile:
            ...
            for line in lines:
                ...
                lemmas = [
                    lemma for (_, lemma) in self.lemmatize(line, best_guess=False)
                ]
