...
from cltk.phonology.akkadian.syllabifier import Syllabifier
    ...
    def find_stress(self, word):
        ...
        for i, syllable in enumerate(word):
            ...
            if len(syllable) == 1:
                ...
                if self._is_consonant(syllable[0]) and self._is_circumflex_vowel(
                    syllable[1]
                ):
                    syllables_stress.append((syllable, "Ultraheavy"))
                    continue
                if (
                    self._is_macron_vowel(syllable[0])
                    or self._is_circumflex_vowel(syllable[0])
                ) and self._is_consonant(syllable[1]):
                    syllables_stress.append((syllable, "Ultraheavy"))
                    continue
            elif len(syllable) == 3:
                if self._is_macron_vowel(syllable[1]) or self._is_circumflex_vowel(
                    syllable[1]
                ):
                    syllables_stress.append((syllable, "Ultraheavy"))
                    continue
