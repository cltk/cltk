...
class Syllabifier:
    ...
    def __init__(self, lang_name):
        ...
        self.all_phonetic_data, self.tamil_phonetic_data, self.all_phonetic_vectors, self.tamil_phonetic_vectors, self.phonetic_vector_length = (
            self.get_lang_data()
        )
    ...
    def get_lang_data(self):
        ...
        # Handle better?
        all_phonetic_data = [
            [int(cell) if cell == "0" or cell == "1" else cell for cell in row]
            for row in all_phonetic_data
        ]
        tamil_phonetic_data = [
            [int(cell) if cell == "0" or cell == "1" else cell for cell in row]
            for row in tamil_phonetic_data
        ]

        all_phonetic_vectors = np.array(
            [row[PHONETIC_VECTOR_START_OFFSET:] for row in all_phonetic_data]
        )
        tamil_phonetic_vectors = np.array(
            [row[PHONETIC_VECTOR_START_OFFSET:] for row in tamil_phonetic_data]
        )
    ...
    def get_phonetic_info(self, lang):
        """For a specified language (lang), it returns the matrix and the vecto
         ...
         """
        phonetic_data = (
            self.all_phonetic_data if lang != LC_TA else self.tamil_phonetic_data
        )
        phonetic_vectors = (
            self.all_phonetic_vectors if lang != LC_TA else self.tamil_phonetic_vectors
        )
    ...
    def is_plosive(self, v):
        ...
        return (
            self.is_consonant(v)
            and self.get_property_vector(v, "consonant_type")[0] == 1
        )
    ...
    def orthographic_syllabify(self, word):
        ...
        for i in range(len(word)):
            ...
            if i + 1 < len(word) and (
                not self.is_valid(p_vectors[i + 1]) or self.is_misc(p_vectors[i + 1])
            ):
                syllables.append(u" ")
                ...
                anu_nonplos = (
                    i + 2 < len(word)
                    and self.is_anusvaar(p_vectors[i + 1])
                    and not self.is_plosive(p_vectors[i + 2])
                )

                anu_eow = i + 2 == len(word) and self.is_anusvaar(p_vectors[i + 1])
                ...
                if self.is_consonant(p_vectors[i + 1]):
                    syllables.append(u" ")
                elif self.is_vowel(p_vectors[i + 1]) and not self.is_dependent_vowel(
                    p_vectors[i + 1]
                ):
                    syllables.append(u" ")
                elif self.is_anusvaar(p_vectors[i + 1]):
                    anu_nonplos = i + 2 < len(word) and not self.is_plosive(
                        p_vectors[i + 2]
                    )
