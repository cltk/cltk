...
from cltkv1.phonology.akkadian.stress import AKKADIAN
from cltkv1.stem.akkadian.stem import ENDINGS, Stemmer
    ...
    def decline_noun(self, noun, gender, mimation=True):
        ...
        for case in self.endings[gender]["plural"]:
            if gender == "m":
                ...
                ending = [
                    x
                    for x in self.endings[gender]["plural"][case]
                    if x[0] == theme_vowel
                ]
                if stem[-2] in self.akkadian["short_vowels"]:
                    form = stem[:-2] + ending[0]
                elif (
                    stem[-1] in self.akkadian["consonants"]
                    and stem[-2] in self.akkadian["macron_vowels"]
                ):
                    form = stem + ending[0]
                else:
