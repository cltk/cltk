...
class TestOrthophonology(unittest.TestCase):
    ...
    def test_vowel_lengthen(self):
        long_i = i.lengthen()
        self.assertEqual(long_i.ipa, i.ipa + 'ː')
    ...
    rule1 = BasePhonologicalRule(
        lambda before, target, after: target == a and before is not None and before[Consonantal] == Consonantal.pos and
                                      after is not None and after[Consonantal] == Consonantal.pos,
    ...
    def test_consonant_geminate(self):
        tt = Consonant(Place.alveolar, Manner.stop, Voiced.neg, 'tː', Geminate.pos)
        self.assertEqual(t.geminate(), tt)
