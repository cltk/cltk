...
class Verse:
    ...
    def __init__(self, original, scansion="", meter=None, valid=False, syllable_count=0,
                 accented="", scansion_notes=None, syllables=None):
    ...
    def __repr__(self) -> str:
        ...
        return '{}(original={!r}, scansion={!r}, meter={!r}, valid={!r}, syllable_count={!r},' \
               ' accented={!r}, scansion_notes={!r}, syllables = {!r})'.format( \
            class_name, self.original, self.scansion, self.meter, self.valid, \
            self.syllable_count, self.accented, self.scansion_notes, self.syllables)

    def __iter__(self):
        return (i for i in (
            self.original, self.scansion, self.meter, self.valid, self.syllable_count,
            self.accented, ", ".join(self.scansion_notes), ", ".join(self.syllables)))

    def __hash__(self):
        base = hash(self.original) ^ hash(self.scansion) ^ hash(self.meter) ^ \
