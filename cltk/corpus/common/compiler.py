...
class Compile(object):
    ...
    def import_corpus(self, corpus_name, corpus_location=None):
        ...
        if corpus_name == 'tlg':
            ...
            orig_files_dir_pos_latin = os.path.join(self.orig_files_dir, 'pos_latin')
    ...
    def get_lacus_curtius_latin_tar(self):
        orig_files_dir_lacus_curtius_latin = os.path.join(self.orig_files_dir, 'lacus_curtius_latin')
        lc_url = 'https://raw.githubusercontent.com/kylepjohnson/corpus_lacus_curtius_latin/master/lacus_curtius.tar.gz'
        s = requests.Session()
        s.mount(lc_url, SSLAdapter(ssl.PROTOCOL_TLSv1))
        ll_tar = s.get(lc_url, stream=True)
    ...
    def get_pos_latin_tar(self):
        orig_files_dir_pos_latin = os.path.join(self.orig_files_dir, 'pos_latin')
        #make compiled files dir for pos_latin
        '''
        compiled_files_dir_pos_latin = os.path.join(self.compiled_files_dir, 'pos_latin')
        if os.path.isdir(compiled_files_dir_pos_latin) is True:
            pass
        else:
            os.mkdir(compiled_files_dir_pos_latin)
        '''
        pg_url = 'https://raw.githubusercontent.com/kylepjohnson/pos_latin/master/pos_latin.tar.gz'
        s = requests.Session()
        s.mount(pg_url, SSLAdapter(ssl.PROTOCOL_TLSv1))
        pg_tar = s.get(pg_url, stream=True)
        pos_latin_file_name = urlsplit(pg_url).path.split('/')[-1]
        pos_latin_file_path = os.path.join(orig_files_dir_pos_latin, pos_latin_file_name)
        try:
