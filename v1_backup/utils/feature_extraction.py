...
from cltk.morphology.utils import get_features, get_pos  # type: ignore[import-not-found]


def cltk_doc_to_features_table(
    cltk_doc: object,
) -> tuple[list[str], list[list[Union[str, int, float, None, np.ndarray]]]]:
    ...
    if not hasattr(cltk_doc, "sentences") or len(getattr(cltk_doc, "sentences")) < 1:
    ...
    for sentence in getattr(cltk_doc, "sentences"):
    ...
    assert (
        len(variable_names) == len(list_of_list_features[0])
